...
def check02(session):
    ...
    try:
        ...
        if last_accessed > timedelta(1):
            # utils.print_pass("No root user access within a day")
            code = "Success"
        else:
            # utils.print_fail("Root user accessd within a day")
            code = "Danger"
        ...
        if root_report[report_cols["MFA"]] == "true":
            # utils.print_pass("MFA is enabled in root user")
            code = "Success"
        else:
            # utils.print_fail("MFA is diabled in root user")
            code = "Danger"
        ...
        if root_report[report_cols["ACCESS_KEY1"]] == "false" and \
            ...
            # utils.print_pass("No access key for root user")
            code = "Success"
        else:
            # utils.print_fail("Access key exists for root user")
            code = "Danger"
    ...
    try:
        ...
        for bucket in buckets:
            try:
                ...
                status = s3.get_public_access_block(Bucket=bucket["Name"])
                # print(self.s3.get_bucket_encryption(Bucket=bucket["Name"]))

                for key, val in status["PublicAccessBlockConfiguration"].items():
                    if val:
                        # utils.print_pass(f"{key} is blocked")
                        pass
                    else:
                ...
                append_table(ret, 0, [bucket["Name"], "일부 허용" if code_bucket == "Danger" else "차단"])                
            
            except botocore.exceptions.ClientError as error:
                if error.response["Error"]["Code"] == "NoSuchPublicAccessBlockConfiguration":
                    # utils.print_pass(error.response["Error"]["Message"])
                    append_table(ret, 0, [bucket["Name"], "차단"])
                else:
                    append_table(ret, 0, [bucket["Name"], error.response["Error"]["Message"]])
                    errorMsg_bucket = error.response["Error"]["Message"]
                    code_bucket = "Error"
...
def check09(session):
    ...
    try:
        ...
        if error.response["Error"]["Code"] == "SubscriptionRequiredException":
            append_table(ret, 0, ["Trusted Advisor를 사용하기 위해서는 AWS 비즈니스 Support 또는 엔터프라이즈 Support 플랜에 가입하여야 합니다."])
            code = "Subscript"
